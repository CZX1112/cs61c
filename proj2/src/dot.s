...
dot:
    li t5, 1
    blt a2, t5, exit_5      # if the length of the vectors < 1, return error code 5
    blt a3, t5, exit_6      # if the stride of either vector < 1, return error code 6
    blt a4, t5, exit_6      # if the stride of either vector < 1, return error code 6
    # Prologue
    addi sp, sp, -4
    sw ra, 0(sp)
...
loop_continue:
    bge t0, a2, loop_end    # if t0 >= a2, go to end
...
loop_end:
    mv a0, t4               # put the result from t4 to a0 to return
    # Epilogue
    lw ra, 0(sp)
    addi sp, sp, 4
